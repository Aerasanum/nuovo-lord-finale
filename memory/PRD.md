...
### Cinematiche 3D — iteration_15 (richiesta utente: "effetto 3D") — testing agent 6/8 flussi OK (2 non esercitati), fix cold-start applicato
- [x] Scena 3D in tempo reale (three.js su expo-gl, stesso stack della mappa) in `src/components/cinematic/scene3d/`: `world.ts` (cielo notte/alba, stelle/luna o sole, terreno con strada, colline, alberi, fog, luci), `army.ts` (formazione low-poly instanced ≤48 figure proporzionale alla composizione reale: fanteria/arcieri/cavalleria/catapulte/carri/bestie, andatura o esultanza), `creatures.ts` (Drago/Angelo/Demone procedurale con ali, coda, fuoco additivo + luce; Falco; `Burst` fuochi), `scenes.ts` (PARTENZA: colonna in marcia, portabandiera con stemma, torcia, keyframe camera, Leggendario che scende e avanza sputando fuoco, Falco in ricognizione; CONQUISTA: castello (skin/livello reali) o Piramide all'alba, vessillo nemico che cade nel fumo, stemma che sale, scintille, superstiti che esultano, giro d'onore del Drago)
- [x] `CinematicGL.tsx`: l'orologio dello show parte **solo quando la GPU ha finito il primo frame** (readPixels 1 px come sync point: i browser trattano `gl.finish()` come flush) → la compilazione shader non consuma la durata; timer "fine" calcolati da quell'istante; grace 6 s se il GL non parte; "Salta" sempre dopo 1 s dal mount; dispose completo (scene + world) allo smontaggio
- [x] Mappa in pausa durante lo show (`setMapRenderHold` in `map3d/engine.ts`: il loop resta vivo, ridisegna al rilascio) — un solo contesto GL attivo, zero GPU sprecata su device
- [x] HUD con scrim a gradiente (alto/basso) per leggibilità su cielo d'alba; scintille più piccole; camera conquista più arretrata; Drago in hover a lato/sopra la colonna
- [x] Galleria **`/cinematics`** (Casata → "Cinematiche"): 8 varianti (standard, Falco, Grande offensiva, Drago, Angelo, Demone, Conquista, Piramide) riprodotte con stemma/alleanza reali; righe disabilitate finché il giocatore non è idratato
- Nota ambiente: nel preview web headless (SwiftShader) il primo frame GL richiede ~8 s (JIT shader) e congela la pagina; su device ~200 ms
