...
- [x] **Login giornaliero** (`domain/daily.py`, `/daily`, `/daily/claim`, `/jobs/{id}/speedup`): ciclo 7 giorni (risorse ×1/×1,5/×2 = 8 % del cap Magazzino della capitale; speed-up 30 min/1 h/2 h; giorno 7 forziere ×3 + 8 h), giorno del Regno UTC+1, serie azzerata se si salta un giorno, claim atomico. Schermata **`/daily`**: key-art del caveau (`vault_0.jpg`), **forziere 3D** (`ChestGL`, expo-gl + three, canvas trasparente a schermo intero con view-offset per posare il forziere sul piedistallo dipinto; coperchio, bagliore, fontana di monete e scintille al reclamo), striscia 7 giorni, card "Hai ricevuto"; auto-apertura una volta per sessione se reclamabile (`DailyGate`, dopo l'intro), pulsanti regalo con pallino su Mappa (HUD) e Città. Minuti di accelerazione spendibili con la pillola **⏩ N min** (`SpeedupButton`) accanto a ogni timer (Città, code, edificio, ricerca, esercito)
