...
#
# First time: bash scripts/qa_backend.sh && DB_NAME=eld_qa python3 scripts/seed_qa_db.py
#
...
if [ "$RESEED" = "1" ]; then
  DB_NAME=eld_qa python3 scripts/seed_qa_db.py --drop
fi
...
exec python3 -m pytest "${ARGS[@]}" -q -o addopts='' -p no:cacheprovider
