...
def demo_auth(session):
    ...
    me = requests.get(api_url("/api/worlds/world_1/me"), headers=headers, timeout=15)
...
class TestWorlds:
    def test_list_worlds(self, session, demo_auth):
        ...
        assert len(worlds) >= 1
        assert any(w["world_id"] == "world_1" for w in worlds)

    def test_join_new_account(self, session, new_account):
        house = f"Casa{uuid.uuid4().hex[:5]}"
        r = session.post(api_url("/api/worlds/world_1/join"), json={"house_name": house}, headers=new_account["headers"], timeout=30)
        assert r.status_code == 200, r.text
        assert "player" in r.json()
        # verify me shows settlements
        me = session.get(api_url("/api/worlds/world_1/me"), headers=new_account["headers"], timeout=15)
    ...
    def test_demo_me(self, session, demo_auth):
        r = session.get(api_url("/api/worlds/world_1/me"), headers=demo_auth["headers"], timeout=15)
        assert r.status_code == 200
        d = r.json()
        assert len(d["settlements"]) >= 1
...
class TestBuildings:
    def test_buildings_catalog(self, session, demo_auth):
        ...
        r = session.get(api_url(f"/api/worlds/world_1/settlements/{sid}/buildings"), headers=demo_auth["headers"], timeout=15)
        assert r.status_code == 200
        d = r.json()
        assert len(d["buildings"]) == 21
    ...
    def test_upgrade_idempotency(self, session, demo_auth):
        sid = demo_auth["settlement_id"]
        # grant plenty of resources + settlement level 3 so Fattoria L2 is allowed
        gr = session.post(api_url("/api/qa/grant"), json={"settlement_id": sid, "level": 3, "resources": {"grain": 500000, "wood": 500000, "clay": 500000, "iron": 500000, "gold": 50000}}, headers=ADMIN_HEADERS, timeout=15)
        assert gr.status_code == 200, gr.text

        idem = f"e2e-fat-{uuid.uuid4().hex[:6]}"
        body = {"idempotency_key": idem}
        r1 = session.post(api_url(f"/api/worlds/world_1/settlements/{sid}/buildings/Fattoria/upgrade"), json=body, headers=demo_auth["headers"], timeout=15)
        assert r1.status_code == 200, r1.text
        job1 = r1.json()["job"]
        r2 = session.post(api_url(f"/api/worlds/world_1/settlements/{sid}/buildings/Fattoria/upgrade"), json=body, headers=demo_auth["headers"], timeout=15)
        assert r2.status_code == 200, r2.text
        assert r2.json()["job"]["job_id"] == job1["job_id"]  # idempotency preserved
        # jobs endpoint
        jr = session.get(api_url(f"/api/worlds/world_1/settlements/{sid}/jobs"), headers=demo_auth["headers"], timeout=15)
        # some backends put jobs on settlement doc; endpoint may or may not exist -> try both
        if jr.status_code == 404:
            jr = session.get(api_url(f"/api/worlds/world_1/settlements/{sid}"), headers=demo_auth["headers"], timeout=15)
        ...
        # cancel
        cr = session.post(api_url(f"/api/worlds/world_1/jobs/{job1['job_id']}/cancel"), json={}, headers=demo_auth["headers"], timeout=15)
    ...
    def test_settlement_upgrade_error_shape(self, session, new_account):
        ...
        r = session.post(api_url(f"/api/worlds/world_1/settlements/{sid}/upgrade"), json={}, headers=new_account["headers"], timeout=15)
...
class TestResearch:
    def test_research_catalog(self, session, demo_auth):
        sid = demo_auth["settlement_id"]
        r = session.get(api_url(f"/api/worlds/world_1/settlements/{sid}/research"), headers=demo_auth["headers"], timeout=15)
        assert r.status_code == 200
        d = r.json()
    ...
    def test_research_prereq_error(self, session, demo_auth):
        ...
        r = session.post(api_url(f"/api/worlds/world_1/settlements/{sid}/research/economy.grain_2/start"), json={}, headers=demo_auth["headers"], timeout=15)
        if r.status_code >= 400:
...
class TestArmy:
    def test_army_catalog(self, session, demo_auth):
        sid = demo_auth["settlement_id"]
        r = session.get(api_url(f"/api/worlds/world_1/settlements/{sid}/army"), headers=demo_auth["headers"], timeout=15)
        assert r.status_code == 200
        d = r.json()
        assert len(d["units"]) == 13
    ...
    def test_recruit_and_finish(self, session, demo_auth):
        ...
        # baseline
        before = session.get(api_url(f"/api/worlds/world_1/settlements/{sid}/army"), headers=demo_auth["headers"], timeout=15).json()
        base_count = before["army"].get("Fanteria", 0)
        r = session.post(api_url(f"/api/worlds/world_1/settlements/{sid}/recruit"), json={"unit": "Fanteria", "count": 5}, headers=demo_auth["headers"], timeout=15)
        assert r.status_code == 200, r.text
        job = r.json()["job"]
        # fast-forward
        ...
        session.post(api_url("/api/qa/scheduler/run"), json={}, headers=ADMIN_HEADERS, timeout=30)
        after = session.get(api_url(f"/api/worlds/world_1/settlements/{sid}/army"), headers=demo_auth["headers"], timeout=15).json()
...
class TestMap:
    def test_chunk_home(self, session, demo_auth):
        ...
        cx, cy = s["x"] // 32, s["y"] // 32
        r = session.get(api_url(f"/api/worlds/world_1/map/chunk/{cx}/{cy}"), headers=demo_auth["headers"], timeout=15)
        assert r.status_code == 200
        d = r.json()
    ...
    def test_chunk_out_of_range(self, session, demo_auth):
        r = session.get(api_url("/api/worlds/world_1/map/chunk/999/999"), headers=demo_auth["headers"], timeout=15)
        assert r.status_code in (400, 404)
        body = r.json()
        assert body.get("code") in ("CHUNK_OUT_OF_RANGE", "NOT_FOUND") or "code" in body

    def test_overview_without_bearer(self, session):
        r = session.get(api_url("/api/worlds/world_1/map/overview"), timeout=15)
        assert r.status_code == 401
        body = r.json()
        assert set(body) >= {"code", "message"}
    ...
    def test_overview_with_bearer(self, session, demo_auth):
        import base64
        r = session.get(api_url("/api/worlds/world_1/map/overview"), headers=demo_auth["headers"], timeout=30)
        assert r.status_code == 200, r.text
        d = r.json()
...
class TestMarch:
    def _find_neutral(self, session, headers, sx, sy):
        ...
        for dx in (-1, 0, 1):
            for dy in (-1, 0, 1):
                r = session.get(api_url(f"/api/worlds/world_1/map/chunk/{cx+dx}/{cy+dy}"), headers=headers, timeout=15)
    ...
    def test_attack_neutral_full_flow(self, session, demo_auth):
        ...
        pv = session.post(api_url("/api/worlds/world_1/marches/preview"), json=body, headers=demo_auth["headers"], timeout=15)
        assert pv.status_code == 200, pv.text
        assert "eta_seconds" in pv.json()
        r = session.post(api_url("/api/worlds/world_1/marches"), json=body, headers=demo_auth["headers"], timeout=15)
        assert r.status_code == 200, r.text
        m = r.json()["march"]
        march_id = m["march_id"]
        # idempotent retry
        r2 = session.post(api_url("/api/worlds/world_1/marches"), json=body, headers=demo_auth["headers"], timeout=15)
        assert r2.status_code == 200 and r2.json()["march"]["march_id"] == march_id
        # baseline battles count
        b_before = session.get(api_url("/api/worlds/world_1/battles"), headers=demo_auth["headers"], timeout=15).json().get("battles", [])
        ...
        session.post(api_url("/api/qa/scheduler/run"), json={}, headers=ADMIN_HEADERS, timeout=30)
        b_after = session.get(api_url("/api/worlds/world_1/battles"), headers=demo_auth["headers"], timeout=15).json().get("battles", [])
        ...
        session.post(api_url("/api/qa/scheduler/run"), json={}, headers=ADMIN_HEADERS, timeout=30)
        b_after2 = session.get(api_url("/api/worlds/world_1/battles"), headers=demo_auth["headers"], timeout=15).json().get("battles", [])
        assert len(b_after2) == len(b_after), "duplicate battle created on scheduler re-run"
        # inbox has a notification
        inbox = session.get(api_url("/api/worlds/world_1/inbox"), headers=demo_auth["headers"], timeout=15).json()
