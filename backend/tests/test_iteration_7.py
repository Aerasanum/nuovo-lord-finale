...
DEMO_SETTLEMENT = "stl_0484bd7cfcbd40dd"
...
def demo(s):
    ...
    me = s.get(api("/api/worlds/world_1/me"), headers=h, timeout=15).json()
...
class TestSkins:
    def test_skins_catalog_shape(self, s, demo):
        r = s.get(api(f"/api/worlds/world_1/settlements/{DEMO_SETTLEMENT}/skins"), headers=demo["headers"], timeout=15)
        assert r.status_code == 200, r.text
        d = r.json()
        assert d.get("current") in ("classic", "sandstone", "royal", "obsidian"), d
        assert isinstance(d.get("level"), int) and d["level"] >= 1
        assert isinstance(d.get("skins"), list) and len(d["skins"]) == 4
        ids = [x["id"] for x in d["skins"]]
        assert ids == ["classic", "sandstone", "royal", "obsidian"]
        thresholds = {x["id"]: x["min_level"] for x in d["skins"]}
        assert thresholds == {"classic": 1, "sandstone": 5, "royal": 10, "obsidian": 20}
        # classic must be unlocked; higher tiers gated by level (demo level should be 3)
        for x in d["skins"]:
    ...
    def test_skin_put_locked_returns_409_with_required_level(self, s, demo):
        ...
        r = s.put(api(f"/api/worlds/world_1/settlements/{DEMO_SETTLEMENT}/skin"), json={"skin": "royal"}, headers=demo["headers"], timeout=15)
    ...
    def test_skin_put_invalid_returns_400(self, s, demo):
        r = s.put(api(f"/api/worlds/world_1/settlements/{DEMO_SETTLEMENT}/skin"), json={"skin": "zzz"}, headers=demo["headers"], timeout=15)
        assert r.status_code == 400, r.text
        assert r.json().get("code") == "INVALID_SKIN"

    def test_skin_put_classic_ok(self, s, demo):
        r = s.put(api(f"/api/worlds/world_1/settlements/{DEMO_SETTLEMENT}/skin"), json={"skin": "classic"}, headers=demo["headers"], timeout=15)
        assert r.status_code == 200, r.text
        d = r.json()
        assert d["current"] == "classic"
        # verify persistence via GET
        g = s.get(api(f"/api/worlds/world_1/settlements/{DEMO_SETTLEMENT}/skins"), headers=demo["headers"], timeout=15).json()
        assert g["current"] == "classic"

    def test_skin_get_unauthenticated_401(self, s):
        r = s.get(api(f"/api/worlds/world_1/settlements/{DEMO_SETTLEMENT}/skins"), timeout=15)
        assert r.status_code == 401, r.text

    def test_skin_put_unauthenticated_401(self, s):
        r = s.put(api(f"/api/worlds/world_1/settlements/{DEMO_SETTLEMENT}/skin"), json={"skin": "classic"}, timeout=15)
        assert r.status_code == 401, r.text
    ...
    def test_skin_foreign_settlement_forbidden(self, s, demo):
        ...
        br = s.get(api("/api/worlds/world_1/battles?limit=30"), headers=demo["headers"], timeout=15).json()
        target_id = None
        ...
        r = s.get(api(f"/api/worlds/world_1/settlements/{target_id}/skins"), headers=demo["headers"], timeout=15)
        assert r.status_code in (403, 404), r.text
        r2 = s.put(api(f"/api/worlds/world_1/settlements/{target_id}/skin"), json={"skin": "classic"}, headers=demo["headers"], timeout=15)
...
class TestBattleHistory:
    ...
    def test_battles_by_settlement_shape(self, s, demo, target_settlement):
        r = s.get(api(f"/api/worlds/world_1/settlements/{target_settlement}/battles?limit=5"), headers=demo["headers"], timeout=15)
        assert r.status_code == 200, r.text
        d = r.json()
    ...
    def test_battles_by_settlement_limit_upper_bound_422(self, s, demo, target_settlement):
        r = s.get(api(f"/api/worlds/world_1/settlements/{target_settlement}/battles?limit=21"), headers=demo["headers"], timeout=15)
        assert r.status_code == 422, r.text

    def test_battles_by_own_settlement_ok(self, s, demo):
        r = s.get(api(f"/api/worlds/world_1/settlements/{DEMO_SETTLEMENT}/battles?limit=5"), headers=demo["headers"], timeout=15)
