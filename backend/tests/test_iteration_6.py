...
class TestHouseCrest:
    def test_house_get_demo(self, s, demo):
        r = s.get(api("/api/worlds/world_1/house"), headers=demo["headers"], timeout=15)
        assert r.status_code == 200, r.text
        d = r.json()
    ...
    def test_house_put_valid_updates(self, s, demo):
        # get catalog first
        cat = s.get(api("/api/worlds/world_1/house"), headers=demo["headers"], timeout=15).json()["catalog"]
        palette = cat["palette"]
        body = {
        ...
        r = s.put(api("/api/worlds/world_1/house"), json=body, headers=demo["headers"], timeout=15)
        assert r.status_code == 200, r.text
        h = r.json()["house"]
    ...
    def test_house_denormalised_on_chunk(self, s, demo):
        ...
        r = s.get(api(f"/api/worlds/world_1/map/chunk/{cx}/{cy}"), headers=demo["headers"], timeout=15)
...
class TestHostileIntel:
    ...
    def duo(self, s):
        ...
        for attempt in range(6):
            ...
            dfd = _register(s, "dfd")
            for who in (atk, dfd):
                r = s.post(api("/api/worlds/world_1/join"), json={"house_name": f"Casa{uuid.uuid4().hex[:5]}"}, headers=who["headers"], timeout=30)
                assert r.status_code == 200, r.text
                me = s.get(api("/api/worlds/world_1/me"), headers=who["headers"], timeout=15).json()
            ...
            pv = s.post(api("/api/worlds/world_1/marches/preview"), json=probe, headers=atk["headers"], timeout=15)
    ...
    def test_attack_and_intel_flow(self, s, duo):
        ...
        pv = s.post(api("/api/worlds/world_1/marches/preview"), json=body, headers=atk["headers"], timeout=15)
        ...
        # launch
        r = s.post(api("/api/worlds/world_1/marches"), json=body, headers=atk["headers"], timeout=15)
        assert r.status_code == 200, r.text
        m = r.json()["march"]
        march_id = m["march_id"]
        ...
        dr = s.get(api("/api/worlds/world_1/marches"), headers=dfd["headers"], timeout=15)
        ...
        ar = s.get(api("/api/worlds/world_1/marches"), headers=atk["headers"], timeout=15).json()
